...
import com.mephone.lovelynote.pen.HardwareButtonListener;
import com.mephone.lovelynote.write.Graphics.Tool;
import com.mephone.lovelynote.write.LinearFilter.Filter;
...
public class HandwriterView
    ...
    public void add(Graphics graphics) {
        if (graphics instanceof Stroke) { // most likely first
            ...
            page.addImage(img);
        } else if (graphics instanceof GraphicsText) {
            GraphicsText text = (GraphicsText) graphics;
            page.addText(text);
        } else
    ...
    public void setToolType(Tool tool) {
        ...
        switch (tool) {
            ...
            case TEXT:
                ...
                if (canvas != null) {
                    center(canvas.getWidth() / 2, canvas.getHeight() / 2);
                }
                break;
            case IMAGE:
...
//		}
    ...
    protected void center(float xCenter, float yCenter) {
